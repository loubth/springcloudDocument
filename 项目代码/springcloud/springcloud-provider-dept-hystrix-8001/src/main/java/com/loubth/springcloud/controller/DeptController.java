...
import com.loubth.springcloud.service.DeptService;
...
public class DeptController {
    ...
    //get方法的备选方案。这个方案更棒，对象能拿到，但是信息是记录了不存在。这个会在get失败时直接被调用，不需要写自己的url绑定
    public Dept hystrixGet(@PathVariable("id") long id) {
        return new Dept().setDeptno(id).setDname("id=>" + id + "，没有对应的信息，null--@Hystrix").setDb_source("this database doesnot exist in Mysql"); //lombok的链式写法支持
    }
}
